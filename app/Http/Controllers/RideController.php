...
class RideController extends APIController
{
  ...
  public function retrieve(Request $request){
    ...
    foreach ($data as $key) {
      ...
      $data[$i]['created_at_human'] = $this->daysDiff($data[$i]['created_at']);
      if($key['payload'] == 'manual'){
        ...
        $data[$i]['from_date_human'] = $this->daysDiff($data[$i]['from_date_time']);
        $data[$i]['to_date_human'] = $this->daysDiff($data[$i]['to_date_time']);

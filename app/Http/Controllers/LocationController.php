...
class LocationController extends APIController
{
  ...
  public function retrieveLocationsOnly(Request $request){
    $data = $request->all();
    $this->response['data'] = Location::select('id', 'code', 'route', 'country', 'locality', 'latitude', 'longitude')->where($data['condition'][0]['column'], '=', $data['condition'][0]['value'])->orderBy('route', 'asc')->get();
    return $this->response();
  }
